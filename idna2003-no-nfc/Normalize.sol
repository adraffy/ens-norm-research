...
contract Normalize is Ownable {
	...
	mapping (uint256 => uint256) _mappedLarge; // 2x128
	mapping (uint256 => uint256) _mappedSmall; // 4x64
	mapping (uint256 => uint256) _valid;
	mapping (uint256 => uint256) _emoji;

	// data is packed as [rule, rule, ...]
	...
	// data is packed as [rule, rule, ...]
	// where rule = [32 byte: bits][2 byte: key] = 34 bytes
	// where key = cp >> 8
	// where valid = (bits & (1 << (cp & 0x3F)) != 0)
			...
			if (temp > 0) { // emoji
				...
				if ((_valid[cp >> 8] & (1 << (cp & 0xFF))) != 0) {
					...
					if (temp != 0) {
						...
						temp = (_mappedSmall[cp >> 2] >> ((cp & 0x3) << 6)) & 0xFFFFFFFFFFFFFFFF;
						if (temp != 0) {
							if (temp < 0xFFFFFF) {
								cps[out++] = uint24(temp);
							} else {
							...
							temp = (_mappedLarge[cp >> 1] >> ((cp & 0x1) << 7)) & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
							if (temp != 0) {
								uint256 size = temp >> 125;
								uint256 end = out + size;
								if (end > cps.length) {
								...
								if (size == 6) {
									while (out < end) {
										cps[out++] = uint16(temp); // could be 20 bit
										temp >>= 16;
									}
								} else {
									while (out < end) {
	...
	function debugSmall(uint256 cp) public view returns (uint256 value) {		
		value = (_mappedSmall[cp >> 2] >> ((cp & 0x3) << 6)) & 0xFFFFFFFFFFFFFFFF;
	}

	function debugLarge(uint256 cp) public view returns (uint256 len, uint256 value) {		
		value = (_mappedLarge[cp >> 1] >> ((cp & 0x1) << 7)) & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
		len = value >> 125;
		value &= 0x1FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
	}
	...
	function encodeUTF8(uint24[] memory cps) public pure returns (string memory s) {
		bytes memory v = new bytes(cps.length << 2); 
		uint256 pos;
		uint256 ptr;
		uint256 buf;
		assembly {
