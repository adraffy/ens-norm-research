...
contract Ethmoji is Ownable {
	...
	function test(string memory name) public view returns (string memory label, uint256 label_hash, bytes memory counts, string memory repeated) {
		...
		if (counts.length == 1) { // all emoji are the same
			repeated = string(readFirstEmojiUnsafe(bytes(name)));
		}
	...
	function readFirstEmojiUnsafe(bytes memory name) private view returns (bytes memory emoji) {
		unchecked {
			uint256 off;
			uint256 ptr;
			emoji = new bytes(64);
			...
			(off, ptr) = processEmoji(off, type(uint256).max, ptr);
			assembly {
				mstore(emoji, sub(ptr, emoji))
			}
		}
	}
